  ...
  useEffect(() => {
    const fetchTrendingNews = async () => {
      try {
        ...
        setTrendingNews(Array.isArray(data) ? data : []);
      } catch (err) {
  ...
  return (
    <aside className="card-editorial p-5">
      ...
      {loading ? (
        ...
        <p className="text-sm text-editorial-muted py-4">No trending articles</p>
      ) : (
            ...
            return (
              <li key={item._id}>
                ...
                >
                  ...
                  <p className="text-caption text-editorial-muted dark:text-zinc-400 mt-1 ml-4">{views} views · {formatDate(item.createdAt)}</p>
                </Link>
              </li>
            );
          })}
